d3.json('/data').then(function(data) {
    ...
    const barMargin = { top: 20, right: 10, bottom: 40, left: 10 };
    const barWidth = 150;
    ...
    // let x = d3.scaleLinear().domain([1, 100]).range([0, width]);
    ...
    function updateAxes(xParameter, yParameter) {
        ...
        const xDomain = xParameter === 'popularity' ? [1, d3.max(data, d => d.popularity_rank)] : [1, d3.max(data, d => d.rank)];
        x.domain(xDomain);
